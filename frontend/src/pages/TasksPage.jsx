        ...
        const applyFilters = async () => {
            try {
                ...
                if (completionFilter !== 'ALL') {
                    const res = await ApiService.getTasksByCompletionStatus(completionFilter === 'COMPLETED')
                    if (res.statusCode === 200) {
                        result = res.data
                    }
                }
                ...
                if (priorityFilter !== 'ALL') {
                    const res = await ApiService.getTasksByPriority(priorityFilter)
    ...
    const toggleComplete = async (taskId, currentStatus) => {
        ...
        if (res.statusCode === 200) {
            setTasks(tasks.map(task => {
                taskId === taskId ? {...tasks, completed: !currentStatus} : task
            }))
        }
